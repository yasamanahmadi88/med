...
class RewritePaletteProvider extends PaletteProvider {
  ...
  getPaletteEntries() {
    ...
    function createTransformerModule(event: any) {
    ...
    assign(actions, {
      ...
      'create.start-event': createAction(elementFactory, create, 'bpmn:StartEvent', 'events', 'bpmn-icon-start-event-none', '开始事件'),
      'create.end-event': createAction(elementFactory, create, 'bpmn:EndEvent', 'events', 'bpmn-icon-end-event-none', '结束事件'),
    });
