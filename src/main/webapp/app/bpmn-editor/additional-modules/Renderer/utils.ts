...
export function isTypedEvent(event: ModdleElement, eventDefinitionType: string, filter?: any): boolean {
  function matches(definition: any, filter: any) {
    return every(filter, function (val: any, key: any) {
      return definition[key] == val;
    });
  }
  return some(event.eventDefinitions, function (definition: any) {
...
// ////////////////// svg 图形绘制部分
// 绘制圆形
export function drawCircle(renderer: any, parentGfx: any, width: any, height: any, offset: any, attrs?: any): SVGElement {
  if (isObject(offset)) {
    attrs = offset;
    offset = 0;
  }
